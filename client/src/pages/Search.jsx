...
export default function Search() {
    ...
    const handleChange = (e) => {
        ...
        const onShowMoreClick = async() => {
            ...
            const res = await fetch(`/api/listing/get?${seacrhQuery}`);
            const data = await res.json();
            if (data.length < 9){
                setShowMore(false);
            }
        ...
        <div className='p-7 border-b-2 md:border-r-2 md:min-h-screen'>
            <form onSubmit={handleSubmit} className='flex flex-col gap-8'>
                <div className='flex items-center gap-2'>
                    ...
                    <input type='text' placeholder='Search...' className="border rounded-lg p-3 w-full" id="searchTerm" value={sidebardata.searchTerm} onChange={handleChange}/>
                </div>
                <div className='flex'>
                    ...
                    <div className='flex gap-2 flex-wrap items-center'>
                        <input type="checkbox" name="" id="offer" className='w-5' onChange={handleChange} checked={sidebardata.offer}/>
                        <span>Offer</span>
                    </div>
                </div>
